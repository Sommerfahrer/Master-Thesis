...
Für eine leichte Wartbarkeit wurde \ac{freertos} weitestgehend in C entwickelt.
...
Für nicht unterstütze Architekturen stellt \ac{freertos} eine Portierungs-Anleitung zur Verfügung, welche als Unterstützung bei der Portierung von \ac{freertos} auf andere Boards dient \autocite{freertos_port}.
...
\begin{itemize}
    ...
    \item \textbf{Suspended}: Die Task wurde temporär deaktiviert.
\end{itemize}
\begin{center}
...
Dieser Interrupt wird standartmäßig jede Millisekunde aufgerufen.
...
Diese Implementierung von \ac{freertos} vermeidet Fehler wie z.B. eine höher priorisierte Task wartet auf einen Wert einer niedrigeren priorisierten Task, wobei die niedere priorisierte Task nie ausgeführt wird.
Bei Queues wird oft das Prinzip \ac{fifo} verwendet, um eine zeitliche Abarbeitung der Daten zu gewährleisten \autocite{freertos_queues}.
\end{document}
